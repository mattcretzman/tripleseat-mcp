...
import { ALL_TOOLS } from "../roles.js";
...
export function dashboardPage(data: DashboardData): string {
  ...
  const activeUserCount = data.users.filter((u: any) => u.is_active).length;

  const toolOptions = ALL_TOOLS.map(
    (t) => `<option value="${t}">${t}</option>`
  ).join("");

  const roleOptions = data.roles
    .map((r: any) => `<option value="${r.id}">${r.name}</option>`)
    .join("");

  const userOptions = data.users
    .map((u: any) => `<option value="${u.id}">${u.name}</option>`)
    .join("");
