...
import { summarizeList, truncateResponse, LEAD_SUMMARY_FIELDS } from "./formatters.js";
...
export const leadTools: ToolDefinition[] = [
  ...
  },
  {
    name: "search_leads",
    description:
      "Search leads by name, email, date, status, or location. Returns summary fields only. Use get_lead for full details.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    execute: async (args) => {
      ...
      return truncateResponse(JSON.stringify(result), "search_leads");
    },
  },
  {
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatGet("/leads/search", params);
      const result = summarizeList(data, LEAD_SUMMARY_FIELDS);
      return truncateResponse(JSON.stringify(result), "list_recent_leads");
    },
  },
  {
    name: "create_lead",
    description:
      "Create a new lead in TripleSeat. Requires first_name, last_name, email_address, and phone_number at minimum.",
    inputSchema: {
      type: "object",
      properties: {
