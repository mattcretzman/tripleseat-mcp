...
import { tripleseatPost } from "../tripleseat.js";
...
export const taskTools: ToolDefinition[] = [
  {
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatPost(`/leads/${lead_id}/tasks`, taskFields);
  ...
  },
  {
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatPost(`/bookings/${booking_id}/tasks`, taskFields);
      return JSON.stringify(data, null, 2);
    },
  },
  {
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatPost(`/contacts/${contact_id}/tasks`, taskFields);
      return JSON.stringify(data, null, 2);
    },
  },
];
