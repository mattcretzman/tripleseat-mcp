...
import { summarizeList, truncateResponse, EVENT_SUMMARY_FIELDS } from "./formatters.js";

export const eventTools: ToolDefinition[] = [
  {
    name: "get_event",
    description:
      "Get full event details from TripleSeat by event ID. Returns BEO data, packages, vendors, status, and optionally financial details.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "search_events",
    description:
      "Search TripleSeat events by name, date range, status, or venue. Returns summary fields only (id, name, status, dates, location, guest count). Use get_event with the event ID for full details.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    execute: async (args) => {
      ...
      return truncateResponse(JSON.stringify(result), "search_events");
    },
  },
  {
    name: "list_upcoming_events",
    description:
      "List events in a date range. Returns summary fields only (id, name, status, dates, location, guest count). Use get_event for full details on any event.",
    inputSchema: {
      type: "object",
      properties: {
        from_date: { type: "string", description: "Start date (MM/DD/YYYY)" },
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatGet("/events/search", params);
      const result = summarizeList(data, EVENT_SUMMARY_FIELDS);
      return truncateResponse(JSON.stringify(result), "list_upcoming_events");
    },
  },
  {
    name: "check_availability",
    description:
      "Check if a specific date is available at a venue by looking for existing events on that date.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "create_event",
    description:
      "Create a new event in TripleSeat. Requires name, event_start, event_end, account_id, contact_id, location_id, room_ids, and status.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    execute: async (args) => {
      const { data } = await tripleseatPost("/events", { event: args });
      return JSON.stringify(data, null, 2);
    },
  },
  {
    ...
    execute: async (args) => {
      ...
      const { data } = await tripleseatPut(`/events/${event_id}`, { event: eventFields });
      return JSON.stringify(data, null, 2);
    },
  },
];
