...
async function storeTokens(tokens) {
    ...
    await (0, db_js_1.query)(`INSERT INTO tripleseat_tokens (id, access_token, refresh_token, expires_at, updated_at)
     VALUES ('primary', $1, $2, $3, NOW())
     ON CONFLICT (id) DO UPDATE SET
       access_token = $1,
       refresh_token = $2,
       expires_at = $3,
       updated_at = NOW()`, [tokens.access_token, tokens.refresh_token || null, expiresAt.toISOString()]);
...
async function getAccessToken() {
    ...
    try {
        const row = await (0, db_js_1.queryOne)(`SELECT access_token, refresh_token, expires_at FROM tripleseat_tokens WHERE id = 'primary'`);
        if (row) {
    ...
    throw new Error("TripleSeat access token expired and refresh failed. Visit /auth/login to re-authorize.");
}
/**
