...
exports.automationTools = [
    {
        ...
        description: "Generate a standard task sequence for a booking based on event date. Uses built-in wedding templates or accepts custom task lists. Automatically calculates due dates relative to the event and skips past-due tasks.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                event_date: {
                    type: "string",
                    description: "The event date (MM/DD/YYYY) — used to calculate due dates. If not provided, will be fetched from the booking.",
                },
                assignee_ids: {
                    type: "array",
                    items: { type: "number" },
        ...
        execute: async (args) => {
            const { booking_id, assignee_ids, site_id, task_type_id, template = "wedding_standard", custom_tasks, } = args;
            let eventDateStr = args.event_date;
            if (!eventDateStr) {
                try {
                    const { data } = await (0, tripleseat_js_1.tripleseatGet)(`/bookings/${booking_id}`);
                ...
                if (!eventDateStr) {
                    return JSON.stringify({
                        error: "Could not determine event date from booking. Please provide event_date explicitly.",
                    });
                }
            }
            ...
            for (const task of tasks) {
                ...
                try {
                    await (0, tripleseat_js_1.tripleseatPost)(`/bookings/${booking_id}/tasks`, {
                        body: task.body,
            ...
            const formattedEventDate = `${eventDate.getMonth() + 1}/${String(eventDate.getDate()).padStart(2, "0")}/${eventDate.getFullYear()}`;
            const result = {
                booking_id,
            ...
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(result, null, 2), "auto_create_tasks");
        },
    },
    {
        name: "batch_update_events",
        description: "Update multiple events at once — status changes, descriptions, guest counts. Processes sequentially with detailed success/failure tracking. Capped at 25 events per batch.",
        inputSchema: {
            type: "object",
            properties: {
                event_ids: {
        ...
        execute: async (args) => {
            ...
            for (const event_id of event_ids) {
                try {
                    await (0, tripleseat_js_1.tripleseatPut)(`/events/${event_id}`, { event: updates });
            ...
            return (0, formatters_js_1.truncateResponse)(JSON.stringify({ total: event_ids.length, succeeded, failed, results }, null, 2), "batch_update_events");
        },
    },
    {
        name: "batch_update_leads",
        description: "Update multiple leads at once — status, additional information, location. Processes sequentially with detailed success/failure tracking. Capped at 25 leads per batch.",
        inputSchema: {
            type: "object",
            properties: {
                lead_ids: {
        ...
        execute: async (args) => {
            ...
            for (const lead_id of lead_ids) {
                try {
                    await (0, tripleseat_js_1.tripleseatPut)(`/leads/${lead_id}`, { lead: updates });
            ...
            const succeeded = results.filter((r) => r.status === "updated").length;
            const failed = results.filter((r) => r.status === "failed").length;
            return (0, formatters_js_1.truncateResponse)(JSON.stringify({ total: lead_ids.length, succeeded, failed, results }, null, 2), "batch_update_leads");
        },
    },
    {
        name: "batch_create_tasks",
        description: "Create the same task across multiple bookings at once. Useful for adding a task to every event in a date range. Processes sequentially with detailed success/failure tracking. Capped at 25 bookings per batch.",
        inputSchema: {
            type: "object",
            properties: {
                booking_ids: {
        ...
        execute: async (args) => {
            ...
            for (const booking_id of booking_ids) {
                try {
                    await (0, tripleseat_js_1.tripleseatPost)(`/bookings/${booking_id}/tasks`, task);
            ...
            const failed = results.filter((r) => r.status === "failed").length;
            return (0, formatters_js_1.truncateResponse)(JSON.stringify({ total: booking_ids.length, succeeded, failed, task_body: task.body, results }, null, 2), "batch_create_tasks");
        },
    },
];
