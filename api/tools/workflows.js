...
function computeAlerts(todayEvents, weekEvents, recentLeads) {
    const alerts = [];
    for (const ev of todayEvents) {
        if (!ev.guest_count) {
            alerts.push(`${ev.name || "Event #" + ev.id} today is missing a final guest count`);
        }
    }
    const now = new Date();
    ...
    if (newLeads.length > 0) {
        alerts.push(`${newLeads.length} new lead(s) in the last 24 hours awaiting first contact`);
    }
    const byLocationAndDay = new Map();
    ...
    for (const [key, evts] of byLocationAndDay) {
        ...
        const sorted = evts
            ...
            .sort((a, b) => new Date(a.event_start).getTime() -
        ...
        for (let i = 0; i < sorted.length - 1; i++) {
            ...
            if (gapMs > 0 && gapHours <= 2) {
                ...
                alerts.push(`${dayStr} has back-to-back events at ${loc} — ${gapHours} hour turnover`);
            }
            else if (gapMs < 0) {
                const loc = sorted[i].location || key.split("::")[0];
                const dayStr = shortDate(new Date(sorted[i].event_start));
                alerts.push(`${dayStr} at ${loc} has overlapping time slots`);
            }
        }
    }
    return alerts;
...
exports.workflowTools = [
    ...
    {
        name: "daily_briefing",
        description: "Morning briefing — today's events, this week's events, recent leads, and alerts. One call to start your day.",
        inputSchema: {
            type: "object",
            properties: {
                location_id: {
        ...
        execute: async (args) => {
            const now = new Date();
            const today = (0, tripleseat_js_1.formatDate)(now);
            const weekEnd = new Date(now);
            weekEnd.setDate(weekEnd.getDate() + 7);
            const weekEndStr = (0, tripleseat_js_1.formatDate)(weekEnd);
            const baseParams = {};
            if (args.location_id)
                baseParams.location_id = args.location_id;
            const [todayRes, weekRes, leadsRes] = await Promise.all([
                (0, tripleseat_js_1.tripleseatGet)("/events/search", {
                    ...baseParams,
                ...
                (0, tripleseat_js_1.tripleseatGet)("/leads/search", {
                    ...baseParams,
                    order: "created_at",
                    sort_direction: "desc",
                }),
            ...
            const briefing = {
                ...
                today: {
                    ...
                    events: todayEvents.map((e) => (0, formatters_js_1.summarizeItem)(e, formatters_js_1.EVENT_SUMMARY_FIELDS)),
                },
                this_week: {
                    event_count: weekEvents.length,
                    events: weekEvents
                        .slice(0, 15)
                        .map((e) => (0, formatters_js_1.summarizeItem)(e, formatters_js_1.EVENT_SUMMARY_FIELDS)),
                },
                recent_leads: {
                    ...
                    leads: recentLeads
                        .slice(0, 10)
                        .map((l) => (0, formatters_js_1.summarizeItem)(l, formatters_js_1.LEAD_SUMMARY_FIELDS)),
                },
                alerts,
            };
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(briefing, null, 2), "daily_briefing");
        },
    },
    // ── 2. week_at_a_glance ────────────────────────────────────────────
    {
        name: "week_at_a_glance",
        description: "Calendar density view for the next 7 days — events grouped by day with conflict detection and turnover warnings.",
        inputSchema: {
            type: "object",
            properties: {
                start_date: {
        ...
        execute: async (args) => {
            ...
            const params = {
                event_start_date: (0, tripleseat_js_1.formatDate)(start),
                event_end_date: (0, tripleseat_js_1.formatDate)(end),
                order: "event_start",
                sort_direction: "asc",
            };
            if (args.location_id)
            ...
            for (const [key, dayEvents] of dayMap) {
                ...
                for (let i = 0; i < sorted.length - 1; i++) {
                    ...
                    if (gapMs < 0) {
                        ...
                        notes.push(`Back-to-back events — ${gapHours} hour turnover between ${endTime} and ${startTime}`);
                    }
                }
                days.push({
                    date: shortDate(d),
                    event_count: dayEvents.length,
                    events: dayEvents.map((e) => (0, formatters_js_1.summarizeItem)(e, formatters_js_1.EVENT_SUMMARY_FIELDS)),
                    notes,
                });
            }
            ...
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(result, null, 2), "week_at_a_glance");
        },
    },
    // ── 3. lead_intake ─────────────────────────────────────────────────
    {
        name: "lead_intake",
        description: "Create a new lead with user-friendly field names and optionally check date availability in one call.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        execute: async (args) => {
            ...
            if (args.event_date)
                leadPayload.event_date = args.event_date;
            if (args.guest_count)
                leadPayload.guest_count = args.guest_count;
            if (args.event_description)
                leadPayload.event_description = args.event_description;
            if (args.location_id)
                leadPayload.location_id = args.location_id;
            if (args.additional_notes)
                leadPayload.additional_information = args.additional_notes;
            const shouldCheck = args.check_availability !== false && args.event_date;
            const leadPromise = (0, tripleseat_js_1.tripleseatPost)("/leads", { lead: leadPayload });
            ...
            if (availRes) {
                ...
                result.availability = {
                    ...
                    existing_events: existingEvents.map((e) => `${e.name || "Event #" + e.id} (${e.event_start || "unknown time"})`),
                };
            }
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(result, null, 2), "lead_intake");
        },
    },
    // ── 4. smart_search ────────────────────────────────────────────────
    {
        name: "smart_search",
        description: "Cross-entity search — searches events, leads, bookings, contacts, and accounts simultaneously for a query.",
        inputSchema: {
            type: "object",
            properties: {
                query: {
        ...
        execute: async (args) => {
            ...
            const fieldMap = {
                events: formatters_js_1.EVENT_SUMMARY_FIELDS,
                leads: formatters_js_1.LEAD_SUMMARY_FIELDS,
                bookings: formatters_js_1.BOOKING_SUMMARY_FIELDS,
                contacts: formatters_js_1.CONTACT_SUMMARY_FIELDS,
                accounts: formatters_js_1.ACCOUNT_SUMMARY_FIELDS,
            };
            const endpointMap = {
            ...
            const searches = types.map(async (type) => {
                try {
                    const { data } = await (0, tripleseat_js_1.tripleseatGet)(endpointMap[type], {
                        query: args.query,
                    });
                    ...
                    return {
                        ...
                        items: items
                            .slice(0, 10)
                            .map((item) => (0, formatters_js_1.summarizeItem)(item, fieldMap[type])),
                    };
                }
                catch {
            ...
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(response, null, 2), "smart_search");
        },
    },
    // ── 5. convert_lead ────────────────────────────────────────────────
    {
        name: "convert_lead",
        description: "Convert a lead into a booking — fetches the lead, extracts details, and creates a linked booking in one step.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                booking_name: {
                    type: "string",
                    description: "Name for the new booking (defaults to lead event_description or contact name)",
                },
                start_date: {
                    type: "string",
                    description: "Booking start date (MM/DD/YYYY). Defaults to lead's event_date.",
                },
                end_date: {
                    type: "string",
                    description: "Booking end date (MM/DD/YYYY). Defaults to start_date.",
                },
                location_id: {
                    type: "number",
        ...
        execute: async (args) => {
            const { data: leadData } = await (0, tripleseat_js_1.tripleseatGet)(`/leads/${args.lead_id}`);
            const lead = leadData?.lead || leadData?.results?.[0] || leadData;
            const startDate = args.start_date || lead.event_date;
            if (!startDate) {
                return JSON.stringify({
                    converted: false,
                    error: "Lead has no event_date and no start_date was provided. Please supply a start_date.",
                    lead: (0, formatters_js_1.summarizeItem)(lead, formatters_js_1.LEAD_SUMMARY_FIELDS),
                });
            }
            const endDate = args.end_date || startDate;
            const locationId = args.location_id || lead.location_id;
            const bookingName = args.booking_name ||
            ...
            if (locationId)
                bookingPayload.location_id = locationId;
            if (lead.guest_count)
                bookingPayload.guest_count = lead.guest_count;
            if (lead.account_id)
                bookingPayload.account_id = lead.account_id;
            if (lead.contact_id)
                bookingPayload.contact_id = lead.contact_id;
            const { data: bookingData } = await (0, tripleseat_js_1.tripleseatPost)("/bookings", {
            ...
            if (!args.start_date) {
                notes.push(`Start date set to ${startDate} from lead's requested event date`);
            }
            ...
            return (0, formatters_js_1.truncateResponse)(JSON.stringify(result, null, 2), "convert_lead");
        },
    },
];
